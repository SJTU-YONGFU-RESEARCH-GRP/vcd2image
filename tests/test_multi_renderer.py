...
class TestMultiFigureRenderer:
    ...
    @patch("vcd2image.core.multi_renderer.SignalPlotter")
    def test_render_categorized_figures_success(self, mock_signal_plotter, tmp_path, caplog) -> None:
    ...
    def test_generate_category_json_no_signals(self, capsys) -> None:
        ...
        with patch('vcd2image.core.multi_renderer.logger') as mock_logger:
            ...
            renderer._generate_category_json(
                ...
                mock_plotter, "test_category", [], Path("/tmp")
    ...
    @patch("vcd2image.core.multi_renderer.SignalPlotter")
    def test_render_categorized_figures_load_data_failure(self, mock_signal_plotter, tmp_path) -> None:
    ...
    @patch("vcd2image.core.multi_renderer.SignalPlotter")
    def test_render_categorized_figures_categorize_failure(self, mock_signal_plotter, tmp_path) -> None:
    ...
    @patch("vcd2image.core.multi_renderer.SignalPlotter")
    def test_render_categorized_figures_with_formats_none(self, mock_signal_plotter, tmp_path, caplog) -> None:
    ...
    def test_generate_enhanced_categorized_plots_svg_format(self, mock_signal_plotter) -> None:
        ...
        mock_logger.info.assert_any_call("SVG format requested for internals but not yet implemented")
    ...
    def test_generate_enhanced_categorized_plots_html_format(self, mock_signal_plotter) -> None:
        ...
        mock_logger.info.assert_any_call("HTML format requested for outputs but not yet implemented")
        mock_logger.info.assert_any_call("HTML format requested for internals but not yet implemented")
    ...
    def test_generate_category_json_success(self, tmp_path) -> None:
        ...
        with patch('vcd2image.core.multi_renderer.logger') as mock_logger:
            renderer._generate_category_json(
                mock_plotter, "test_category", [], tmp_path
            )
    ...
    def test_generate_category_json_with_real_data(self, timer_vcd_file, tmp_path) -> None:
        ...
        renderer._generate_category_json(
            plotter, "test_category", ["tb_timer/clock"], output_path
        )
    ...
    def test_generate_category_json_wave_extractor_failure(self, mock_wave_extractor, tmp_path, caplog) -> None:
        """Test _generate_category_json with WaveExtractor failure (lines 205-207)."""
        renderer = MultiFigureRenderer()

        mock_plotter = Mock()
        mock_plotter.vcd_file = str(tmp_path / "test.vcd")

        # Mock WaveExtractor to return failure code
        ...
        with caplog.at_level(logging.WARNING):
            renderer._generate_category_json(
                mock_plotter, "test_category", ["signal1"], tmp_path
            )

        # Should warn about WaveExtractor failure
        assert "Failed to generate JSON for test_category: WaveExtractor returned 1" in caplog.text

    @patch("vcd2image.core.multi_renderer.WaveExtractor")
    def test_generate_category_json_exception_handling(self, mock_wave_extractor, tmp_path, caplog) -> None:
        ...
        # Mock WaveExtractor to raise an exception
        mock_wave_extractor.side_effect = RuntimeError("Test exception")
        ...
        # Should warn about general exception
        assert "Failed to generate JSON for test_category: Test exception" in caplog.text
    ...
    def test_render_enhanced_plots_with_golden_references_exception(self, tmp_path) -> None:
        ...
        with patch("vcd2image.core.multi_renderer.SignalPlotter", return_value=mock_plotter_instance):
            renderer = MultiFigureRenderer()
    ...
    @patch("vcd2image.core.multi_renderer.SignalPlotter")
    def test_render_enhanced_plots_with_golden_references_load_failure(self, mock_signal_plotter, tmp_path) -> None:
    ...
    @patch("vcd2image.core.multi_renderer.SignalPlotter")
    def test_render_enhanced_plots_with_golden_references_categorize_failure(self, mock_signal_plotter, tmp_path) -> None:
    ...
    @patch("vcd2image.core.multi_renderer.SignalPlotter")
    def test_render_enhanced_plots_with_golden_references_generate_plots_failure(self, mock_signal_plotter, tmp_path) -> None:
    ...
    def test_render_auto_plot_load_data_failure(self, mock_signal_plotter, tmp_path) -> None:
        ...
        result = renderer.render_auto_plot(
            vcd_file=str(tmp_path / "test.vcd"),
            output_file=str(tmp_path / "auto_plot.png")
        )
